...
function renderStep3(){
  if (!flatRates || flatRates.length === 0){
    return `<div class="card"><div class="top-actions"><div></div><button type="button" class="primary empty-cta-highlight" onclick="return miqAddFlatFromButton(event)" id="addFlatRateBtn">Add position</button></div><div class="flat-empty-state"><div class="flat-empty-copy"><div class="flat-empty-title">No flat-rate positions yet</div><div class="flat-empty-sub">Click <strong>Add position</strong> to start calculating the margin for this project.</div></div></div>
      ${navButtons()}
    </div>`;
  }

  const hasOpen = flatRates.some(p => !p._collapsed);
  return `<div class="card"><div class="top-actions" style="justify-content:flex-end;"><button type="button" onclick="return miqAddFlatFromButton(event)">Add position</button></div>
...
function addFlat(){
  flatRates.forEach(p => { if (p) p._collapsed = true; });
...
try { window.addFlat = addFlat; } catch(_) {}
