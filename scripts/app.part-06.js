...
(function(){
  ...
  window.renderStep3 = function(){
    if (!flatRates || !flatRates.length){
      return '<div class="card"><div class="flat-rate-top-controls flat-rate-viewby-steps"><div class="viewby-static-slot" aria-hidden="true">View by</div><button type="button" class="active" disabled>All</button><button type="button" disabled>Provider type</button><button type="button" disabled>Contract classification</button><button type="button" class="add-position-btn" onclick="addFlat()" id="addFlatRateBtn">Add position</button></div><div class="flat-empty-state"><div class="flat-empty-copy"><div class="flat-empty-title">No flat-rate positions yet</div><div class="flat-empty-sub">Click <strong>Add position</strong> to start calculating the margin for this project.</div></div></div>' + navButtons() + '</div>';
    }
    ...
    var topControls = hasOpen ? '' : '<div class="flat-rate-top-controls flat-rate-viewby-steps">' + buttonBar(window.flatRatesGroupBy || '', 'setFlatRatesGroupBy') + '<button type="button" class="add-position-btn" onclick="addFlat()">Add position</button></div>';
    var list = '';
    if (hasOpen) {
