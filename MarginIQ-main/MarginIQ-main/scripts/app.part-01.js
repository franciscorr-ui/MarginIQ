...
// ===== MarginIQ fix: expose flatRates to later render patches =====
// Several later scripts read window.flatRates, while the real state is the
// top-level let variable flatRates. Without this bridge, Add position can
// update the data but the Flat-rate screen still renders as empty.
(function(){
  try {
    var descriptor = Object.getOwnPropertyDescriptor(window, 'flatRates');
    if (!descriptor || !descriptor.__miqFlatRatesStateBridge) {
      var bridgeDescriptor = {
        configurable: true,
        enumerable: false,
      ...
      bridgeDescriptor.__miqFlatRatesStateBridge = true;
      Object.defineProperty(window, 'flatRates', bridgeDescriptor);
    }
  } catch (_) {}
})();
